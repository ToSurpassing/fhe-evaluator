...
void RunOneMode(const std::string& modeName, ScalingTechnique scalTech) {
    std::cout << "\n============================================================\n";
    std::cout << "[BASELINE] eager double-square aggregate\n";
    ...
    // Aggregate after both branches are materialized
    auto t12 = Clock::now();
    ...
    std::cout << "\nsummary:\n";
    std::cout << "  eager aggregate total time = " << std::scientific << totalTime << " s\n";
    std::cout << "  relin count                = 2\n";
    std::cout << "  rescale count              = 2\n";
    std::cout << "  final level                = " << ct_sum->GetLevel() << '\n';
    std::cout << "  final noiseScaleDeg        = " << ct_sum->GetNoiseScaleDeg() << '\n';
}
