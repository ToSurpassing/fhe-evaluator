...
CoeffPattern MakeInternalPatternA() {
    return CoeffPattern{
...
CoeffPattern MakeInternalPatternB() {
    return CoeffPattern{
...
std::vector<size_t> OuterCandidates() {
    return {0, 16};
}

std::vector<size_t> HighCandidates() {
    return {12, 8, 4};
}

std::vector<size_t> LowCandidates() {
    return {1, 2, 3};
}

bool TryDecomposePower(size_t finalPower, InnerTerm& term, size_t& outerPower) {
...
InternalBsgsPlan ReferencePlanForPattern(const CoeffPattern& pattern) {
...
InternalBsgsPlan GenerateInternalPlanFromCoeffs(const CoeffPattern& pattern) {
...
std::map<size_t, double> PlanCoeffMap(const InternalBsgsPlan& plan) {
...
void ValidateGeneratedPlanMatchesCoeffs(const InternalBsgsPlan& plan, const CoeffPattern& pattern) {
...
void RunOneMode(const InternalBsgsPlan& plan, const std::string& modeName, ScalingTechnique scalTech) {
    std::cout << "\n============================================================\n";
    std::cout << "[EVALUATOR] generated fixed tiny internal-BSGS prototype\n";
    std::cout << "[PROJECT DECISION] generate t=2, B=4 internal plans from whitelisted coefficient patterns\n";
