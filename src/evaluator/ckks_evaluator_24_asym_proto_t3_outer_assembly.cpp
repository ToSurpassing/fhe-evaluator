...
void RunOneMode(const std::string& modeName, ScalingTechnique scalTech) {
    std::cout << "\n============================================================\n";
    std::cout << "[EVALUATOR] asymmetric prototype t=3 outer assembly\n";
    std::cout << "[PROJECT DECISION] fixed two-block BSGS-shaped path, not a general planner\n";
    ...
    if (eagerErr >= 1e-8 || lazyErr >= 1e-8) {
        throw std::runtime_error("asym proto t3 outer assembly error threshold failed");
    }
    if (lazy.stats.relinCount > eager.stats.relinCount ||
        lazy.stats.rescaleCount > eager.stats.rescaleCount) {
        throw std::runtime_error("asym proto t3 outer assembly lazy switch count is worse than eager");
    }
}
