...
OuterAssemblyPlan GenerateOuterAssembly24PlanFromCoeffs(const CoeffPattern& pattern) {
    if (!SameCoeffPattern(pattern, MakeOuterAssembly24CoeffPattern())) {
        throw std::runtime_error("GenerateOuterAssembly24PlanFromCoeffs: unsupported coefficient pattern");
    }
    ...
    const auto rules = OuterAssembly24Rules();
    for (const auto& [power, coeff] : pattern.coeffsByPower) {
        ...
        if (rule.outerPower == 0) {
            ...
            throw std::runtime_error("GenerateOuterAssembly24PlanFromCoeffs: unsupported outer power");
        }
    }
    return plan;
}
...
void RunOneMode(const OuterAssemblyPlan& plan, const std::string& modeName, ScalingTechnique scalTech) {
    ...
    std::cout << "[PROJECT DECISION] generate one whitelisted plan from coefficients, not arbitrary layouts\n";
    ValidatePlan(plan);
...
int main() {
    try {
        ...
        const auto pattern = MakeOuterAssembly24CoeffPattern();
        const auto id = SelectWhitelistedPlanForCoeffs(pattern);
        if (id != PlanId::OuterAssembly24) {
            throw std::runtime_error("unexpected plan id for generated plan smoke");
        }
        const auto referencePlan = MakeWhitelistedPlan(id);
        const auto plan = GenerateOuterAssembly24PlanFromCoeffs(pattern);
