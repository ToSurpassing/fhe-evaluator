...
const std::vector<T3Term> kTerms = {
...
EvalOutput RunGroupedLazy(CC cc,
    ...
    auto t1 = Clock::now();
    out.trace.push_back(Capture("grouped-lazy", "final folded t=3 sum materialized",
...
void RunOneMode(const std::string& modeName, ScalingTechnique scalTech) {
    std::cout << "\n============================================================\n";
    std::cout << "[EVALUATOR] asymmetric prototype t=3 grouped fold\n";
    std::cout << "[PROJECT DECISION] fixed small t=3 grouped-fold prototype, not a full planner\n";
    std::cout << "[DECOMP] component powers={x,x^2,x^4,x^8}; each term multiplies three components\n";
    std::cout << "[TERMS] x^7, x^8, x^9, x^11 via four t=3 decomposed products\n";
    ...
    std::cout << "  note                      = lazy folds " << kTerms.size()
              << " degree-3 raw products before one materialization\n";

    if (eagerErr >= 1e-8 || lazyErr >= 1e-8) {
        throw std::runtime_error("asym proto t3 grouped fold error threshold failed");
    }
    if (lazy.stats.relinCount > eager.stats.relinCount ||
        lazy.stats.rescaleCount > eager.stats.rescaleCount) {
        throw std::runtime_error("asym proto t3 grouped fold lazy switch count is worse than eager");
    }
}
