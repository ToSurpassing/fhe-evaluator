    ...
    return OuterAssemblyPlan{
        ...
        {1, 2, 4, 8},
        {
            BlockSpec{
                ...
                0,
                {
                    T3Term{1, 1, 8, -0.11},   // x^10
                    T3Term{2, 4, 8, 0.28},    // x^14
                    T3Term{4, 4, 8, -0.19},   // x^16
                },
            },
            BlockSpec{
                "block1",
                8,
                {
                    T3Term{1, 2, 8, 0.17},    // x^11, then outer *x^8 => x^19
                    T3Term{2, 4, 8, -0.13},   // x^14, then outer *x^8 => x^22
                    T3Term{4, 8, 8, 0.09},    // x^20, then outer *x^8 => x^28
                },
            },
        },
    };
...
std::vector<DecompositionRule> OuterAssembly24Rules() {
    return {
        DecompositionRule{7, 1, 2, 4, 0},
        DecompositionRule{8, 2, 2, 4, 0},
        DecompositionRule{9, 1, 4, 4, 0},
        DecompositionRule{11, 1, 2, 8, 0},
        DecompositionRule{15, 1, 2, 4, 8},
        DecompositionRule{16, 2, 2, 4, 8},
        DecompositionRule{17, 1, 4, 4, 8},
    };
}

std::vector<DecompositionRule> OuterPattern25Rules() {
    return {
        DecompositionRule{10, 1, 1, 8, 0},
        DecompositionRule{14, 2, 4, 8, 0},
        DecompositionRule{16, 4, 4, 8, 0},
        DecompositionRule{19, 1, 2, 8, 8},
        DecompositionRule{22, 2, 4, 8, 8},
        DecompositionRule{28, 4, 8, 8, 8},
    };
}

const DecompositionRule& FindRule(const std::vector<DecompositionRule>& rules, size_t finalPower) {
    const auto it = std::find_if(rules.begin(), rules.end(), [finalPower](const DecompositionRule& rule) {
        return rule.finalPower == finalPower;
    });
    if (it == rules.end()) {
        throw std::runtime_error("FindRule: missing decomposition rule for x^" +
                                 std::to_string(finalPower));
    }
    return *it;
}

OuterAssemblyPlan GeneratePlanFromRules(const CoeffPattern& pattern,
                                        const std::string& planName,
                                        const std::vector<DecompositionRule>& rules) {
    OuterAssemblyPlan plan{
        planName,
        {1, 2, 4, 8},
        {
            BlockSpec{"block0", 0, {}},
    ...
    for (const auto& [power, coeff] : pattern.coeffsByPower) {
        const auto& rule = FindRule(rules, power);
        const T3Term term{rule.a, rule.b, rule.c, coeff};
        if (rule.outerPower == 0) {
            plan.blocks[0].terms.push_back(term);
        }
        else if (rule.outerPower == 8) {
            plan.blocks[1].terms.push_back(term);
        }
        else {
            throw std::runtime_error("GeneratePlanFromRules: unsupported outer power");
        }
    }
    return plan;
}

OuterAssemblyPlan GeneratePlanFromCoeffs(const CoeffPattern& pattern) {
    if (SameCoeffPattern(pattern, MakeOuterAssembly24CoeffPattern())) {
        return GeneratePlanFromRules(
            pattern,
            "generated_outer_assembly_24_shape",
            OuterAssembly24Rules());
    }
    if (SameCoeffPattern(pattern, MakeOuterPattern25CoeffPattern())) {
        return GeneratePlanFromRules(
            pattern,
            "generated_outer_pattern_25_shape",
            OuterPattern25Rules());
    }
    throw std::runtime_error("GeneratePlanFromCoeffs: unsupported coefficient pattern");
}
...
std::map<size_t, double> PlanCoeffMap(const OuterAssemblyPlan& plan) {
    std::map<size_t, double> out;
    for (const auto& block : plan.blocks) {
        for (const auto& term : block.terms) {
            out[TermPower(term) + block.outerMultiplierPower] += term.coeff;
        }
    }
    return out;
}

void ValidateGeneratedPlanMatchesCoeffs(const OuterAssemblyPlan& plan, const CoeffPattern& pattern) {
    constexpr double kTol = 1e-12;
...
std::string PowerListLabel(const BlockSpec& block) {
    std::string out;
    for (size_t i = 0; i < block.terms.size(); ++i) {
        if (i != 0) {
            out += ",";
        }
        out += "x^" + std::to_string(TermPower(block.terms[i]) + block.outerMultiplierPower);
    }
    return out;
}
...
void RunOneMode(const OuterAssemblyPlan& plan, const std::string& modeName, ScalingTechnique scalTech) {
    std::cout << "\n============================================================\n";
    std::cout << "[EVALUATOR] asymmetric generated-plan t=3 executor\n";
    std::cout << "[PROJECT DECISION] generate whitelisted plans from coefficients, not arbitrary layouts\n";
    ValidatePlan(plan);
    std::cout << "[PLAN] " << plan.name << '\n';
    std::cout << "[POLY] P(x)=block0(x)+block1(x)*z, z=x^8\n";
    std::cout << "[block0] powers " << PowerListLabel(plan.blocks[0]) << "\n";
    std::cout << "[block1*z] powers " << PowerListLabel(plan.blocks[1]) << "\n";
...
int main() {
    try {
        const auto knownPatterns = KnownCoeffPatterns();
        std::cout << "[KNOWN_PATTERNS] " << knownPatterns.size() << "\n";

        for (const auto& pattern : knownPatterns) {
            const auto id = SelectWhitelistedPlanForCoeffs(pattern);
            const auto referencePlan = MakeWhitelistedPlan(id);
            const auto plan = GeneratePlanFromCoeffs(pattern);
            ...
            std::cout << "[GENERATED_PLAN] " << plan.name << '\n';
