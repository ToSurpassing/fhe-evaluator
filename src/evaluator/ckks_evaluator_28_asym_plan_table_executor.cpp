...
void RunOneMode(const OuterAssemblyPlan& plan, const std::string& modeName, ScalingTechnique scalTech) {
    std::cout << "\n============================================================\n";
    std::cout << "[EVALUATOR] asymmetric plan-table t=3 executor\n";
    std::cout << "[PROJECT DECISION] execute a small table of validated fixed plans, not a general planner\n";
    ValidatePlan(plan);
...
int main() {
    try {
        const std::vector<OuterAssemblyPlan> plans{
            MakeOuterAssembly24Plan(),
            MakeOuterPattern25Plan(),
        };
