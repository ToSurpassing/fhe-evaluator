...
std::vector<PlanId> WhitelistedPlanIds() {
    return {
        PlanId::OuterAssembly24,
        PlanId::OuterPattern25,
    };
}
...
void RunOneMode(const OuterAssemblyPlan& plan, const std::string& modeName, ScalingTechnique scalTech) {
    std::cout << "\n============================================================\n";
    std::cout << "[EVALUATOR] asymmetric whitelisted-planner t=3 executor\n";
    std::cout << "[PROJECT DECISION] generate only whitelisted fixed plans, not arbitrary layouts\n";
    ValidatePlan(plan);
...
int main() {
    try {
        for (const auto id : WhitelistedPlanIds()) {
            const auto plan = MakeWhitelistedPlan(id);
