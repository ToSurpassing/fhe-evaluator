...
constexpr size_t kOuterPower = 8;
...
std::vector<double> RefOuterBlock1() {
    std::vector<double> ref(kInput.size(), 0.0);
    for (const auto& term : kBlock1.terms) {
        const size_t power = TermPower(term) + kOuterPower;
        for (size_t i = 0; i < kInput.size(); ++i) {
            ref[i] += term.coeff * PlainPower(kInput[i], power);
        }
    }
...
std::vector<double> RefPolynomial() {
    auto ref = RefBlock(kBlock0);
    const auto outer = RefOuterBlock1();
...
EvalOutput RunExpandedEager(CC cc,
    ...
    auto block0 = EvalBlockExpandedEager(cc, sk, kBlock0, components, out.trace, out.stats);
    auto block1 = EvalBlockExpandedEager(cc, sk, kBlock1, components, out.trace, out.stats);
    out.value = AssembleOuter(cc, sk, "expanded-eager", block0, block1,
                              FindPower(components, kOuterPower), out.trace, out.stats);
    return out;
}
...
EvalOutput RunGroupedLazy(CC cc,
    ...
    auto block0 = EvalBlockGroupedLazy(cc, sk, kBlock0, components, out.trace, out.stats);
    auto block1 = EvalBlockGroupedLazy(cc, sk, kBlock1, components, out.trace, out.stats);
    out.value = AssembleOuter(cc, sk, "grouped-lazy", block0, block1,
                              FindPower(components, kOuterPower), out.trace, out.stats);
    return out;
}
...
void RunOneMode(const std::string& modeName, ScalingTechnique scalTech) {
    std::cout << "\n============================================================\n";
    std::cout << "[EVALUATOR] asymmetric prototype t=3 outer pattern\n";
    std::cout << "[PROJECT DECISION] second fixed two-block BSGS-shaped pattern, not a general planner\n";
    std::cout << "[POLY] P2(x)=block0(x)+block1(x)*z, z=x^8\n";
    std::cout << "[block0] powers x^10,x^14,x^16\n";
    std::cout << "[block1*z] powers x^19,x^22,x^28\n";
    ...
    if (eagerErr >= 1e-8 || lazyErr >= 1e-8) {
        throw std::runtime_error("asym proto t3 outer pattern error threshold failed");
    }
    if (lazy.stats.relinCount > eager.stats.relinCount ||
        lazy.stats.rescaleCount > eager.stats.rescaleCount) {
        throw std::runtime_error("asym proto t3 outer pattern lazy switch count is worse than eager");
    }
}
