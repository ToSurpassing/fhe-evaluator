...
void RunOneMode(const OuterAssemblyPlan& plan, const std::string& modeName, ScalingTechnique scalTech) {
    std::cout << "\n============================================================\n";
    std::cout << "[EVALUATOR] asymmetric coefficient-pattern planner t=3 executor\n";
    std::cout << "[PROJECT DECISION] match only known coefficient patterns, not arbitrary layouts\n";
    ValidatePlan(plan);
